...
uploaded_files = st.file_uploader(
    "Upload image or pdf invoices. Total size should not exceed 250MB.",
...
if len(uploaded_files) > 0:
    total_size_mb = round(sum(file.size for file in uploaded_files) / (1024 * 1024), 2)
    # As the batch processing API allows a maximum of 256MB payload per batch, the total size of uploaded files is restricted to 250MB
    # The remaining 6MB is reserved for the system prompt that is sent with each request in the batch
    if total_size_mb <= 250.0:
        submit = st.button("Submit")
    else:
        st.warning("Total size of all files cannot exceed 250MB, please delete some files and try again")
...
if submit:
    ...
    try:
        file_name = str(datetime.now().strftime("%d-%b-%Y %I_%M_%S_%p"))
        book_path = os.path.join(os.getcwd(), "excel_files", f"{file_name}.xlsx")
